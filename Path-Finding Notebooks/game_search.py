...
def main(tf, source_path, path_selection, custom):
    global num_zeroes, degrees, point_stor, colors, col_i, fig, ax, fill_image, fill_x, fill_y, prev_fill_color, dist, fill, flood_fill, grid, valid_paths, get_neighbors, heuristic, sec_start, sec_end
    for i in range(len(point_stor)):
        x, y = point_stor[i]
        point_stor[i] = (x + tf, y  + tf)
    ...
    for col_sec in routes[path_selection]:
        ...
        def find_loop():
            global fill_image, valid_paths, get_neighbors, heuristic
        ...
        if start_point != end_point:
            ...
            good_path = chaikin_smooth(good_path)
        else:
        ...
        output_paths[col_sec] = {'path': 0, 'midpoint': 0}
    ...
    for color, path in output_paths.items():
        ...
        print(path['midpoint'])
        point_stor.append((path['midpoint'][0],path['midpoint'][1]))
        degrees[point_stor.index(start_point)] += 1
        degrees[point_stor.index(end_point)] += 1
        degrees.append(2)
        # plt.scatter(sec_end[0], sec_end[1], color='lime')
        

    return plt.figure(str(color))
plt.close('all')
...
fig = main(0, source_path, 0, '')
new_img_stor = r"C:\Users\2000039241\OneDrive - Fulton County Schools\Desktop\tempG.png"
plt.savefig(new_img_stor, format='png', bbox_inches='tight', pad_inches = 0, dpi = 100*1.12903226)
redraw_dots()
temp_image = Image.open(new_img_stor)
temp_image = ImageOps.expand(temp_image,border=100,fill= (240, 240, 240))
temp_image.save(new_img_stor)


temp_image = Image.open(new_img_stor)
plt.close('all')

fig2 = main(100, source_path, 2, str(new_img_stor))
print('well')
redraw_dots()
plt.show()